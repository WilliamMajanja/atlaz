...
export default function QuickActions() {
  ...
  const { currentAnalysis, setCurrentAnalysis, saveAnalysis, setIsAnalyzing } = useAnalysisStore();
  ...
  return (
    ...
    >
      ...
      {selectedPoint && !currentAnalysis && (
        <div className="px-4 pt-3">
          <button
            onClick={handleRunAnalysis}
            className="w-full rounded-xl text-[11px] font-semibold py-2.5 px-4 transition-all duration-200 text-white hover:scale-[1.01] active:scale-[0.99]"
            style={{
              background: "linear-gradient(135deg, #06b6d4, #0891b2)",
              boxShadow: "0 0 16px -4px rgba(6, 182, 212, 0.3)",
            }}
          >
      ...
      {currentAnalysis && (
        <div className="px-4 pt-3">
          <div className="rounded-lg p-2.5 mb-2" style={{ background: "rgba(6, 182, 212, 0.06)", border: "1px solid rgba(6, 182, 212, 0.12)" }}>
            <p className="text-[10px] text-cyan-300 font-medium">Analysis ready</p>
            <p className="text-[9px] text-slate-500 mt-0.5">{currentAnalysis.neighbourhood ?? "Unknown location"}</p>
          </div>
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 rounded-lg text-[10px] font-semibold py-2 transition-all text-white hover:scale-[1.01]"
              style={{
                background: saved
            ...
            <Link
              href="/analysis"
              className="flex-1 rounded-lg text-[10px] font-semibold py-2 transition-all text-white text-center hover:scale-[1.01]"
              style={{
      ...
      <div className={`grid grid-cols-1 sm:grid-cols-2 gap-px ${selectedPoint || currentAnalysis ? "pt-2" : ""}`} style={{ background: "rgba(30, 41, 59, 0.4)" }}>
